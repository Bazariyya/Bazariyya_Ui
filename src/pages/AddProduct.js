    ...
    async function fetchData() {
      const [, res] = await tryCatch(
        getProductAttributeValues({
          pageCount: 0,
          pageIndex: 0,
          productAttributeDefinitionId: item.id,
        })
      );
...
const AddProductComp = ({ location, categories }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      if (
        data.productAttributes.length === 0 ||
        data.productAttributes.length !== productAttribute.value.length ||
        data.productAttributes.filter((i) => i.value === "").length > 0
      ) {
        addToast("Lütfen hayvan bilgilerini doldurun.", {
          appearance: "error",
          autoDismiss: true,
          autoDismissTimeout: 3000,
        });
  ...
  const handleChange = (e, item) => {
    ...
    const findAttr = copyProductAttributes.findIndex(
      (i) => i.productAttributeDefinitionId === item.id
    );
        ...
        return (
          <div className="billing-info">
            ...
            <input
              ...
              type="number"
              name={item.header}
              value={formik.values.productAttributes[item.id]?.value}
              onChange={(e) => handleChange(e, item)}
            />
